...
public sealed class SalesChatPluginTool
{
    ...
    public async Task<string> SummarizeAccountProfile( IMcpServer thisServer,string input, CancellationToken cancellationToken)
    {
    ...
    public async Task<string> GetAccountNews(IMcpServer thisServer, string input, CancellationToken cancellationToken)
    {
        return await CallAgent("DraupAccountNews", input, thisServer.ServerOptions.ServerInfo.Name);
    }
    ...
    public async Task<string> GetAccountCompetitor(IMcpServer thisServer, string input, CancellationToken cancellationToken)
    {
        return await CallAgent("Account Competitor", input, thisServer.ServerOptions.ServerInfo.Name);
    }

    [McpServerTool, Description("Fetches and displays the list of account team members working on an account")]
    public async Task<string> GetAccountTeam(IMcpServer thisServer, string input, CancellationToken cancellationToken)
    {
        return await CallAgent("GetAccountTeamM365", input, thisServer.ServerOptions.ServerInfo.Name);
    }
    /*
