...
public sealed class ExecutionHostTools
{
    ...
    public async Task<HttpResponseData> Account_Competitor(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
    ...
    public async Task<HttpResponseData> GetPartnerDetails(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> CEHub_Copilot(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> MI_Summary(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> Deal_Assistance_Programs_Recommender(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> ACR(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> LeadingPrompts(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> Generic_Opportunity_Plugin(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> Commercial_Licensing_QA(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> Account_V2(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> Content(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> Earnings_Estimation(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> GetEngagedContacts(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> SellerICGuide(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> FetchRecommendationsV2(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> Pre_meeting_Brief(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> Commercial_Executive_Plugin(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> RAIN_Copilot(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> GenericMilestonePlugin(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> CXP_Billed_Pipeline_Skill(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> Github_Account_Skill(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> ROTHV3(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> Opportunity_Actions(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> CommercialExecutiveActionsPlugin(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> Deal_Assistance_Program_Guide(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> GenericContactPlugin(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> Realized_Value_Catalog(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
    ...
    public async Task<HttpResponseData> GetRecommendationSummary(McpServer thisServer, string userPrompt, string context, CancellationToken cancellationToken)
    {
        var token = thisServer?.ServerOptions?.ServerInfo?.Name ?? string.Empty;
